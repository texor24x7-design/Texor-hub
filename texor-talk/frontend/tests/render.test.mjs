...
console.log('\n── captions ──');
{
  const { CaptionOverlay } = await import('@/components/CaptionOverlay');
  const { TranscriptPanel } = await import('@/components/TranscriptPanel');

  const LINES = [
    {
      key: 'tx-s:1', texorId: 'tx-s', name: 'surya', text: 'endhuko telidu.',
      language: 'te', confidence: 0.8, final: true, at: new Date(),
    },
    {
      key: 'tx-j:1', texorId: 'tx-j', name: 'john', text: "I don't know either.",
      language: 'en', confidence: 0.9, final: false, at: new Date(),
    },
  ];

  const overlay = render('CaptionOverlay', h(CaptionOverlay, { lines: LINES, self: 'tx-s' }));
  if (overlay.html) {
    check('CaptionOverlay', true);
    check('it attributes each line to whoever said it', overlay.html.includes('john'), overlay.html);
    // Your own captions read "You", the way your own chat messages do.
    check('and calls you "You"', overlay.html.includes('You'), overlay.html);
    check('a sentence still being spoken is marked', overlay.html.includes('meet__caption--interim'), overlay.html);
    /**
     * A caption layer that does not announce itself is invisible to a screen
     * reader — which is a large part of who captions are for.
     */
    check('it is announced politely rather than silently', overlay.html.includes('aria-live'), overlay.html);
  }

  // The empty case is the one a component like this usually crashes on.
  const quiet = render('CaptionOverlay with nothing said', h(CaptionOverlay, { lines: [], self: 'tx-s' }));
  if (quiet.html !== null) check('CaptionOverlay draws nothing when nobody has spoken', quiet.html === '', quiet.html);

  const USER = { texorId: 'tx-s', displayName: 'surya', picture: '' };

  const panel = render('TranscriptPanel', h(TranscriptPanel, {
    lines: LINES, user: USER, code: 'abc-defg-hij', captions: true, stored: true,
  }));
  if (panel.html) {
    check('TranscriptPanel', true);
    check('it says the meeting is being transcribed', /being transcribed/i.test(panel.html), panel.html.slice(0, 400));
    check('and offers the file', panel.html.includes('transcript.txt'), panel.html.slice(0, 600));
  }

  /**
   * Captions on, storage off, is a real configuration — an organisation can
   * allow people to follow along without keeping a record — and the panel must
   * say so rather than implying a transcript exists.
   */
  const unstored = render('TranscriptPanel without storage', h(TranscriptPanel, {
    lines: LINES, user: USER, code: 'abc-defg-hij', captions: true, stored: false,
  }));
  if (unstored.html) {
    check('it says when nothing is being kept', /Nothing is being stored/i.test(unstored.html), unstored.html.slice(0, 400));
    check('and does not offer a download there is no file for', !unstored.html.includes('transcript.txt'));
  }

  const off = render('TranscriptPanel with captions off', h(TranscriptPanel, {
    lines: [], user: USER, code: 'abc-defg-hij', captions: false, stored: false,
  }));
  if (off.html) check('it explains that a host can turn captions on', /host can turn them on/i.test(off.html), off.html.slice(0, 400));
}
