...
```
browser            notes backend              accounts.texor.app
   │                      │                            │
   ├─ click "Continue" ──▶│                            │
...
(`notes_sid`) and nothing else. Access and refresh tokens never reach the
...
Texor owns identity. Texor Notes owns notes, labels and API keys. The bridge is a local `users`
...
`POST /api/auth/logout` revokes the Texor Notes session, then returns Texor's
