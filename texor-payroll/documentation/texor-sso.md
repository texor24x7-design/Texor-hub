...
```
browser            payroll backend            accounts.texor.app
   │                      │                            │
   ├─ click "Continue" ──▶│                            │
...
| `GET /api/auth/callback` | Exchanges the code, verifies the ID token, opens a Texor Payroll session |
| `POST /api/auth/logout` | Revokes the session, returns Texor's end-session URL |
| `GET /api/auth/me` | The signed-in user, or `null` |

## What Texor Payroll trusts
...
(`payroll_sid`) and nothing else. Access and refresh tokens never reach the
...
Texor owns identity. Texor Payroll owns employees and pay runs. The bridge is a local `users`
...
The local record caches the name and picture so listing a hundred payslips does
...
`POST /api/auth/logout` revokes the Texor Payroll session, then returns Texor's
