...
console.log('\n── who may read one ──');
{
  const mine = { ownerTexorId: 'tx-me', visibility: 'private' };
  const theirsPrivate = { ownerTexorId: 'tx-you', visibility: 'private' };
  const theirsShared = { ownerTexorId: 'tx-you', visibility: 'meeting' };

  check('my own note', server.canRead(mine, 'tx-me'));
  check('somebody else’s private note, even from the same meeting',
    !server.canRead(theirsPrivate, 'tx-me', { wasInMeeting: true }));
  check('a shared note, if I was in the meeting',
    server.canRead(theirsShared, 'tx-me', { wasInMeeting: true }));
  check('not a shared note from a meeting I was not in',
    !server.canRead(theirsShared, 'tx-me', { wasInMeeting: false }));
  check('nobody reads anything without an identity', !server.canRead(mine, null));

  check('only the owner writes', server.canWrite(mine, 'tx-me') && !server.canWrite(theirsShared, 'tx-me'));
}
