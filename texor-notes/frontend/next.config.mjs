 ...
 *
 * That is how a shared meeting link in Texor Talk came to advertise its preview
 * image at `http://localhost:3002/opengraph-image`, which no crawler can fetch.
 * The same trap is waiting here for every note link somebody sends.
 *
...
const nextConfig = {
  ...
  env: {
    ...
    NEXT_PUBLIC_API_ORIGIN: normalise(process.env.NEXT_PUBLIC_API_ORIGIN) || 'http://localhost:4004',
    ...
    NEXT_PUBLIC_NOTES_ORIGIN: resolveOrigin({
      configured: process.env.NEXT_PUBLIC_NOTES_ORIGIN,
      fallback: DEV ? 'http://localhost:3004' : '',
    }),
  },
};
