...
```
browser            talk backend            accounts.texor.app
   │                      │                            │
   ├─ click "Continue" ──▶│                            │
...
(`talk_sid`) and nothing else. Access and refresh tokens never reach the
...
Texor owns identity. Texor Talk owns channels and messages. The bridge is a local `users`
...
The local record caches the name and picture so listing a thousand messages does
...
`POST /api/auth/logout` revokes the Texor Talk session, then returns Texor's
